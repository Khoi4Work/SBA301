    ...
    return (
        <div className="animate-fade-in pb-12 w-full">
            ...
            <section className="space-y-4 mb-12">
                ...
                <div
                    className="h-[1px] w-full bg-gradient-to-r from-secondary/50 via-secondary/10 to-transparent"></div>
            </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                <div
                    ...
                    <p className="text-on-surface-variant text-xs uppercase tracking-widest font-semibold group-hover:text-secondary transition-colors">Tổng
                        Học Giả</p>
            ...
            {/* Main Table Section */}
            <div
                className="bg-surface-container-lowest border border-secondary/10 overflow-hidden shadow-lg shadow-surface-lowest">
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                        <tr className="bg-surface-container-high/30 border-b border-secondary/20">
                            <th className="px-6 py-5 font-semibold text-secondary/80 text-[11px] uppercase tracking-widest">Học
                                giả
                            </th>
                            ...
                            <th className="px-6 py-5 font-semibold text-secondary/80 text-[11px] uppercase tracking-widest">Ngày
                                tham gia
                            </th>
                            <th className="px-6 py-5 font-semibold text-secondary/80 text-[11px] uppercase tracking-widest text-right">Hành
                                động
                            </th>
                        </tr>
                        </thead>
                            ...
                            >
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-4">
                                        <div
                                            className="w-12 h-12 bg-surface border border-secondary/30 flex items-center justify-center overflow-hidden shrink-0 group-hover:border-secondary transition-colors">
                                            <img
                                                src={
                                                    user.avatarUrl ||
            ...
            person_off
                ...
                <div className="px-6 py-4 bg-surface flex justify-between items-center border-t border-secondary/10">
                    <p className="text-xs text-on-surface-variant font-medium italic opacity-80">
                        {loading
                            ? "Đang tải học giả..."
                            : `Hiển thị ${users.length} / ${totalElements} học giả — Trang ${currentPage + 1} / ${totalPages || 1}`
                        }
                    </p>
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="p-1 border border-secondary/20 text-on-surface-variant rounded transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:border-secondary hover:text-secondary"
                        >
                        ...
                        {Array.from({length: totalPages}, (_, i) => i)
                            .filter(i => {
                                ...
                                return i === 0 || i === totalPages - 1 || Math.abs(i - currentPage) <= 1;
                            })
                            ...
                            .map((item, idx) =>
                                item === '...' ? (
                                    <span key={`ellipsis-${idx}`} className="text-secondary/50 mx-1">...</span>
                                ) : (
                                    <button
                                        key={item}
                                        onClick={() => goToPage(item)}
                                        disabled={loading}
                                        className={`px-3 py-1 border text-xs font-bold rounded transition-colors disabled:cursor-not-allowed ${
                                            item === currentPage
