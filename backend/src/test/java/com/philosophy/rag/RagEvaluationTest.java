...
public class RagEvaluationTest {
        ...
        void ragAnswerShouldBeRelevantAndGrounded(RagTestCase testCase) throws InterruptedException {
                ...
                List<Document> contextDocuments = (List<Document>) chatResponse.getMetadata()
                ...
                EvaluationResponse factCheckResponse = factCheckingEvaluator.evaluate(factCheckRequest);
        ...
        static Stream<RagTestCase> philosophyQuestions() {
                ...
                // new RagTestCase("Thực chất và nguyên nhân của hiện tượng tha hóa con người
                // trong xã hội có phân chia giai cấp là gì?"),
                // new RagTestCase("Quần chúng nhân dân đóng vai trò như thế nào trong tiến
                // trình phát triển của lịch sử xã hội?"),
                // new RagTestCase("Trong tư tưởng Hồ Chí Minh và quan điểm của Đảng Cộng sản
                // Việt Nam hiện nay, con người giữ vị trí và vai trò gì trong công cuộc đổi
                // mới?")
                );
        }
